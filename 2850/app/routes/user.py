...
# KHÔNG prefix ở đây (prefix được đặt tại app.include_router)
...
def login_user(request: UserLogin):
    ...
    try:
        ...
        user = db_query(
            "SELECT user_id, user_name, user_pass FROM users WHERE user_id=%s AND user_pass=%s",
